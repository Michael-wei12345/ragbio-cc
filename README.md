...
- 论文元数据来自 OpenAlex、PubMed 和 ClinicalTrials.gov；多条检索通道先形成较大的元数据/摘要候选池，再去重和保守初筛。
- 在线全文来源依次覆盖 Europe PMC、OpenAlex TEI、OpenAlex locations、Unpaywall、Semantic Scholar 和开放 PDF。
- 全文按章节和段落解析；PDF 证据尽量保留页码。
- 标题/摘要阶段只做 `Likely / Unclear / Background / Explicit mismatch` 资格分层，不产生最终相关度；无摘要或低置信度记录不会被自动排除。
- 最多 180 篇进入全局证据评分，其中前 120 篇尝试获取全文，其余使用摘要；所有页面共享同一最终顺序，翻页不再调用模型。
...
在搜索栏中可以直接描述研究目标、时间范围、研究对象和开放获取要求。RagBio 会调用设置中启用的大模型，提取 PICO/PECO、研究类型和来源专用检索式，再并行检索 OpenAlex、PubMed 与 ClinicalTrials.gov。OpenAlex 每条通道最多获取 200 条，PubMed 每条通道最多获取 500 条；这些首先只是便宜的元数据和摘要，不会立刻下载全文。

不同来源的记录按 DOI、PMID 和标准化标题融合去重。系统会从少量相关 Review/Meta-analysis 做一轮向前及向后引用追踪，然后在最多 480 条记录上进行保守的标题/摘要资格初筛。初筛不是旧版摘要排序：它不显示相关度、不决定最终顺序，只区分 `Likely`、`Unclear`、`Background` 和高置信度 `Explicit mismatch`；缺少摘要、判断失败或低置信度冲突一律保留为 `Unclear`。

最多 180 篇进入证据准备和一次性全局排序。只有前 120 篇自动尝试获取合法全文，其余论文使用摘要参加同一轮评分，从而扩大召回而不把全文下载、本地 embedding 和 CPU 工作量按候选池规模放大。最终仅显示相关度不低于 5% 的论文；未成功评分的记录不会伪装成低相关度。
...
全文段落检索、结构化证据卡和统一评分在搜索时一次完成。每篇全文最多向模型提供章节分布合理的 6 个段落；没有合法全文或单篇自动获取超过 6 秒时使用摘要。正常排序直接采用 AI 的全局分数，本地规则不再混合加权：核心人群、干预或结局明确不匹配时把分数限制到 4；证据卡明确支持的原始结果研究最低保护为 70，避免一次异常模型评分把直接证据降到低相关区。最终数组、分数和证据等级保存在搜索历史中，翻页只读取数组切片与缓存。

搜索历史同时保存全局评分指纹。指纹覆盖评分规则版本、研究问题、AI 厂商与模型、候选顺序、摘要和实际证据段落；重复搜索得到完全相同的评分输入时直接复用原有证据卡、分数和顺序。候选、证据、模型或评分规则任一变化都会重新评分，API Key 不写入指纹。

当前 10 篇系统综述、72 个目标研究单元的 blind benchmark 在 480 候选池和最终 5% 阈值后的端到端召回均为 70/72（97.2%），已召回研究在 AI@180 和阈值阶段的保留率均为 100%；详见 [systematic-review search benchmark](Benchmarks/SystematicReview/README.md)。

“仅开放获取”和“起始年份”是可逆的结果筛选器。候选发现始终从 OpenAlex、PubMed、ClinicalTrials.gov 及引用追溯中广泛获取论文，并完成同一次资格初筛和全局排序；搜索前勾选只决定完成后最初显示的子集。搜索完成后调整或关闭筛选会立即重新切片已排序数组，并恢复同一次搜索中符合当前条件的论文，不会再次请求数据库、下载全文或调用模型。

如果统一 AI 评分失败，RagBio 会保留本地证据卡顺序并明确显示“统一评分未完成”；重试复用候选、全文和段落缓存，不重新请求数据库。
