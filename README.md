...
- 支持关键词搜索和 AI 搜索；AI 只把自然语言描述转换成可审查的 OpenAlex 检索计划。
- OpenAlex 检索词会经过本地规则清洗和同义词扩展，覆盖常见疾病、药物、数据库缩写和编码系统。
- AI 搜索支持 DeepSeek、OpenAI、Anthropic Claude 和 Google Gemini，凭据保存在本机应用配置中。
- 在线扫描支持 `Use / Maybe / Exclude` 临时决策；这些决策只属于当前 research scan，不等于收藏。
- 可以从当前扫描生成 deterministic Evidence Table，并在此基础上生成带 `work_id` 支撑的 Field Scan Report。
...
在搜索栏切换到“AI 搜索”后，可以直接描述研究目标、时间范围、研究对象和开放获取要求。RagBio 会调用设置中启用的大模型，生成英文检索词、年份、排序和开放获取筛选，再从 OpenAlex 获取最多 50 篇候选论文。
...
搜索结果可以用 `Use`、`Maybe`、`Exclude` 做当前扫描内的筛选。这个决策是为了临时整理一次研究主题，不会把论文保存到全局文库；全局保存仍然使用“收藏”。

在详情页的“扫描工作台”可以生成 Evidence Table：

- 如果用户标了 `Use` 或 `Maybe`，表格只包含这些论文。
- 如果还没有任何决策，表格会基于当前排序生成，并显示 warning。
- `Exclude` 永远不会进入表格。
...
项目会保存当前 query、排序和筛选、搜索结果、AI 分数和理由、全文 summary、scan decisions、Evidence Table、Field Scan Report 和项目更新时间。项目不会保存 API Key、翻译缓存、正在加载的任务状态、出版社凭据或 raw full-text body；全文正文仍然按需从本机 FullText cache 恢复。

打开项目时，RagBio 会取消旧后台任务、递增当前搜索 generation、清空 transient loading state，然后恢复项目中的搜索和扫描状态，不会自动重跑昂贵 AI 任务。
